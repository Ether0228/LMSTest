...
def start_missing_sync():
    ...
    # 从花名册 S1～S6 所属课程字段构建选课索引
    # student_enrollments: {name: {(course, sem_short), ...}}
    # roster_name_to_id:   {name: record_id}
    SEMESTER_COLS = ["S1", "S2", "S3", "S4", "S5", "S6"]
    student_enrollments = {}
    roster_name_to_id = {}
    for s_rec in roster:
        ...
        for sem in SEMESTER_COLS:
            raw = str(s_f.get(f"{sem}所属课程", "") or "").strip()
            for course in filter(None, (c.strip() for c in raw.splitlines())):
                student_enrollments.setdefault(name, set()).add((course, sem))
    print(f">>> 花名册选课：{len(student_enrollments)} 名学生有选课记录")
    ...
    for a_rec in lib:
        ...
        # 从作业学期字段提取短码：'2025-S4' → 'S4'
        a_sem_raw = str(a_f.get("学期", "")).strip()
        m = re.search(r'(S\d+)$', a_sem_raw)
        a_sem_short = m.group(1) if m else ""

        for s_name, s_enroll in student_enrollments.items():
            s_id = roster_name_to_id.get(s_name)

            # 判断逻辑：学生在对应学期选了这门课
            if a_sem_short:
                enrolled = (a_course, a_sem_short) in s_enroll
            else:
                # 作业无学期标签：只要学生在任意学期选了该课即算
                enrolled = any(c == a_course for c, _ in s_enroll)
