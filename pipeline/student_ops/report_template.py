...
def render_courses(courses: list[dict[str, Any]]) -> str:
    ...
    return "<div class='course-stories'>" + "".join(
        f"<article class='course-story'><div><div class='course-code'>{e(c.get('code'))}</div><h3>{e(c.get('title'))}</h3><div class='course-time'>{e(c.get('session_summary'))}</div></div><div class='story-grid'><div class='story-block'><h4>实际授课内容<span class='annotation annotation--fact'>确认事实</span></h4><p>{e(c.get('actual_content'))}</p></div><div class='story-block'><h4>课堂互动<span class='annotation annotation--human'>老师确认</span></h4><p>{e(c.get('confirmed_interaction')) if c.get('interaction_confirmation_status') == '已确认' else '本周无已确认互动事实。'}</p></div><div class='story-block'><h4>学校的下一步支持<span class='annotation annotation--human'>老师确认</span></h4><p>{e(c.get('confirmed_support')) if c.get('support_confirmation_status') == '已确认' else '学校支持安排待老师确认。'}</p></div></div></article>" for c in visible
    ) + "</div>"
...
def render_grades(series: list[dict[str, Any]]) -> str:
    ...
    summaries = [course.get("summary") for course in series if course.get("summary_confirmation_status") == "已确认" and course.get("summary")]
    ...
    summary = "；".join(summaries) or "仅展示近期已评分任务，暂无已确认的事实性成绩说明。"
    ...
    reflection = f"<aside class='reflection'><h3>事实性成绩说明与学校支持</h3><div class='reflection-item'><strong>已确认成绩事实</strong><p>{e(summary)}</p></div><div class='reflection-item'><strong>已确认学校支持</strong><p>{e(support)}</p></div></aside>"
