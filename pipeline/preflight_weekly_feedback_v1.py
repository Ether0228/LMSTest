...
def main() -> int:
    ...
    parser.add_argument("--schoology-snapshot", type=Path, help="已验证的只读 Schoology latest.json；可替代本机 Cookie 作为本次预检的数据源")
    ...
    if args.schoology_snapshot:
        try:
            ...
            parser.error(f"无法读取 Schoology 快照：{exc}")
        env["SCHOOLOGY_SNAPSHOT"] = bool(snapshot.get("course_sections")) and bool(snapshot.get("grade_items"))
