...
function showError(message) {
  ...
  error.innerHTML = `
    <div class="error-screen__inner">
      ...
      <a class="error-screen__link" href="/${location.search}">返回现网版</a>
    </div>
  `;
}
...
async function loadDashboardData(params) {
  ...
  if (cached) {
    ...
    fetch(`/api/dashboard?t=${encodeURIComponent(params.tenant)}&student=${encodeURIComponent(params.student)}`)
  ...
  const resp = await fetch(`/api/dashboard?t=${encodeURIComponent(params.tenant)}&student=${encodeURIComponent(params.student)}`);
...
(function init() {
  ...
  loadDashboardData(params).catch((error) => {
    console.warn("[preview-terminal] API failed, fallback to mock:", error);
    renderMockPreview();
  });
})();
